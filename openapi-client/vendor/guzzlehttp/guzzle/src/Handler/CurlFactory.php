...
use Axytos\FinancialServices\GuzzleHttp\Promise\FulfilledPromise;
use Axytos\FinancialServices\GuzzleHttp\Promise\PromiseInterface;
use Axytos\FinancialServices\GuzzleHttp\Psr7\LazyOpenStream;
...
class CurlFactory implements CurlFactoryInterface
{
     ...
     */
    public static function finish($handler, $easy, $factory)
    {
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf)
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \Axytos\FinancialServices\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \Axytos\FinancialServices\GuzzleHttp\Psr7\Utils::streamFor($sink);
